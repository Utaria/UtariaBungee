...
public class AntiBotManager extends AbstractManager {
	...
	public AntiBotManager() {
		...
		ProxyServer.getInstance().getScheduler().runAsync(UtariaBungee.getInstance(), this::_loadBlackList);
	}
	...
	private boolean isProtectionEnabled() {
		return ConfigTableAccessor.getBoolean("antibot_protection_enabled");
	}
	...
	private void _refreshLoginPool() {
		...
		if (!this.protectionActivated && nbPlayers > AUTHORIZED_PLAYER_IN_PERIOD) {
			...
			this.protectionTask = new AntiBotProtectionTask(this, PROTECTION_TIME_SEC);
		}
	}
