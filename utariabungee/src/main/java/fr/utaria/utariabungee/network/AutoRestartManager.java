...
public class AutoRestartManager extends AbstractManager implements Runnable {
	...
	public AutoRestartManager() {
		...
		// Lancement de la tâche du gestionnaire ...!
		TaskUtil.scheduleSyncRepeatingTask("autoRestartTask", this, 20L, 10 * 20L);
	}
	...
	public void doRestart() {
		...
		UUtil.kickAllPlayers(Config.autoRestartMessage);
		...
		UtariaBungee.getInstance().getInstance(ProxyManager.class).getServerWithName("survie").restart();
	...
	private void _checkForDisable() {
		boolean active = ConfigTableAccessor.getBoolean("autorestart");
	...
	private void _reloadRefreshHour() {
		String restartHour = ConfigTableAccessor.getString("autorestart_hour");

		if (restartHour != null && !this._lastRestartHour.equals(restartHour)) {
			Config.autoRestartTime = restartHour;

			this._restartHour = UUtil.timeToInt(Config.autoRestartTime);
			this._reloadUpTime();
	...
	private void _nextBroadcast() {
		...
		for (int time : this.BROADCAST_AT) {
			if (this._upTime / 1000.0 <= time && !UUtil.intArrayContains(this.broadcasts, time)) {
		...
		ProxyServer.getInstance().broadcast(TextComponent.fromLegacyText(Config.ERROR_PREFIX + "Le serveur va redémarrer automatiquement dans §6" + castEnd + "§c."));
	...
	private boolean _isBroadcastNeeded(int upTime) {
		...
		for (int time : this.BROADCAST_AT) {
			...
			if (upTime / 1000.0 <= time && !UUtil.intArrayContains(this.broadcasts, time))
				return true;
